...
elseif (($todate-$fromdate)>$nodays) {
  $error = "Allowed Leave limit exceeded, you applied for ".($todate-$fromdate)." days";
}
else{
...
.succWrap{
 ...
 <div class="input-field col  s12">
   <?php
   $sql = "SELECT (12-sum(t1.diff)) as count from (SELECT (ToDate-FromDate) as diff from tblleaves where empid=:eid && status=1) as t1";
   $query = $dbh -> prepare($sql);
   ...
   $results=$query->fetchAll(PDO::FETCH_OBJ);
     ...
     ?>
   Number of leaves left for this Year - <?php echo date("Y") ?>&nbsp;:&nbsp;<h5>
   <?php
   if($result->count=="")
    echo "12";
  else
    echo $nodays = $result->count; ?></h5><?php
}
else{
  ?>
  <script> alert("You do not have any leaves left for this year!!")</script>
  <!--meta http-equiv="refresh" content="0;url=leavehistory.php"-->
